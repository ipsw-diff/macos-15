## UserNotificationsKit

> `/System/iOSSupport/System/Library/PrivateFrameworks/UserNotificationsKit.framework/Versions/A/UserNotificationsKit`

```diff

-941.6.6.0.0
-  __TEXT.__text: 0x5ce34
-  __TEXT.__auth_stubs: 0x1ba0
-  __TEXT.__objc_methlist: 0x2c0c
-  __TEXT.__const: 0x1f04
-  __TEXT.__cstring: 0x3364
-  __TEXT.__gcc_except_tab: 0x1b4
-  __TEXT.__oslogstring: 0x2a4c
+941.6.10.0.0
+  __TEXT.__text: 0x5dae8
+  __TEXT.__auth_stubs: 0x1c00
+  __TEXT.__objc_methlist: 0x2c14
+  __TEXT.__const: 0x1ef4
+  __TEXT.__cstring: 0x3394
+  __TEXT.__gcc_except_tab: 0x1b8
+  __TEXT.__oslogstring: 0x2a7c
   __TEXT.__constg_swiftt: 0xc70
-  __TEXT.__swift5_typeref: 0x3c44
+  __TEXT.__swift5_typeref: 0x3c98
   __TEXT.__swift5_reflstr: 0x675
   __TEXT.__swift5_fieldmd: 0x8b0
   __TEXT.__swift5_builtin: 0x64
   __TEXT.__swift5_assocty: 0x210
-  __TEXT.__swift5_capture: 0x564
+  __TEXT.__swift5_capture: 0x590
   __TEXT.__swift5_proto: 0x15c
   __TEXT.__swift5_types: 0xbc
   __TEXT.__swift_as_entry: 0x28
   __TEXT.__swift_as_ret: 0x1c
   __TEXT.__swift5_protos: 0x8
-  __TEXT.__unwind_info: 0x14c0
-  __TEXT.__eh_frame: 0xc38
+  __TEXT.__unwind_info: 0x14f8
+  __TEXT.__eh_frame: 0xc60
   __TEXT.__objc_classname: 0x57e
-  __TEXT.__objc_methname: 0x8df2
+  __TEXT.__objc_methname: 0x8e63
   __TEXT.__objc_methtype: 0x1072
-  __TEXT.__objc_stubs: 0x43a0
-  __DATA_CONST.__got: 0x6c0
+  __TEXT.__objc_stubs: 0x43e0
+  __DATA_CONST.__got: 0x6d0
   __DATA_CONST.__const: 0x4a0
   __DATA_CONST.__objc_classlist: 0x148
   __DATA_CONST.__objc_catlist: 0x18
   __DATA_CONST.__objc_protolist: 0xb8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x1c60
+  __DATA_CONST.__objc_selrefs: 0x1c70
   __DATA_CONST.__objc_protorefs: 0x38
   __DATA_CONST.__objc_superrefs: 0x98
-  __AUTH_CONST.__auth_got: 0xde0
-  __AUTH_CONST.__const: 0x19f8
+  __AUTH_CONST.__auth_got: 0xe10
+  __AUTH_CONST.__const: 0x1ae8
   __AUTH_CONST.__cfstring: 0x18e0
   __AUTH_CONST.__objc_const: 0x53d0
   __AUTH.__objc_data: 0xd18
-  __AUTH.__data: 0xc78
+  __AUTH.__data: 0xc68
   __DATA.__objc_ivar: 0x2d4
-  __DATA.__data: 0x1568
+  __DATA.__data: 0x1598
   __DATA.__bss: 0x2c70
   __DATA.__common: 0x1b0
   - /System/Library/Frameworks/Combine.framework/Versions/A/Combine

   - /usr/lib/swift/libswiftsimd.dylib
   - /usr/lib/swift/libswiftsys_time.dylib
   - /usr/lib/swift/libswiftunistd.dylib
-  Functions: 2091
-  Symbols:   2675
-  CStrings:  1978
+  Functions: 2114
+  Symbols:   2682
+  CStrings:  1982
 
Symbols:
+ -[NCNotificationDispatcher _loadNotificationsImmediatelyIfNeededForSectionIdentifier:]
+ _objc_msgSend$_loadNotificationsImmediatelyIfNeededForSectionIdentifier:
+ _objc_msgSend$dispatcher:supportSectionPreloadForSectionIdentifier:
+ _symbolic Say_____G So17OS_dispatch_queueC8DispatchE10AttributesV
+ _symbolic So19LSApplicationRecordC
+ _symbolic So31NCNotificationSourceCategorizerCXMT
+ _symbolic _____Sg So28NCNotificationSourceCategoryV
CStrings:
+ "_loadNotificationsImmediatelyIfNeededForSectionIdentifier:"
+ "bundleIDToCategoryMapQueue"
+ "dispatcher:supportSectionPreloadForSectionIdentifier:"
+ "load Notifications for %{public}@ immediately"
```
