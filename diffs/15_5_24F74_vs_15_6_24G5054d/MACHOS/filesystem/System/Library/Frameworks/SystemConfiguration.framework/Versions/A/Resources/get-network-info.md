## get-network-info

> `/System/Library/Frameworks/SystemConfiguration.framework/Versions/A/Resources/get-network-info`

### Sections with Same Size but Changed Content

- `__TEXT.__swift5_entry`

```diff

-1351.120.3.0.0
-  __TEXT.__text: 0x7a48
-  __TEXT.__auth_stubs: 0x7c0
-  __TEXT.__const: 0x142
-  __TEXT.__cstring: 0x1a45
-  __TEXT.__swift5_typeref: 0xe2
-  __TEXT.__objc_methname: 0xfd
+1351.140.5.0.0
+  __TEXT.__text: 0xca80
+  __TEXT.__auth_stubs: 0xa70
+  __TEXT.__const: 0x682
+  __TEXT.__cstring: 0x1ad5
   __TEXT.__swift5_entry: 0x8
-  __TEXT.__constg_swiftt: 0xa4
-  __TEXT.__swift5_reflstr: 0x8d
-  __TEXT.__swift5_fieldmd: 0x80
+  __TEXT.__swift5_typeref: 0x274
+  __TEXT.__constg_swiftt: 0x190
+  __TEXT.__swift5_reflstr: 0x2ad
+  __TEXT.__swift5_fieldmd: 0x1d8
+  __TEXT.__objc_methname: 0x18d
+  __TEXT.__swift5_builtin: 0x28
+  __TEXT.__swift5_assocty: 0x60
+  __TEXT.__swift5_proto: 0x54
+  __TEXT.__swift5_types: 0x1c
   __TEXT.__oslogstring: 0x3
-  __TEXT.__swift5_proto: 0x4
-  __TEXT.__swift5_types: 0x8
-  __TEXT.__unwind_info: 0x148
-  __TEXT.__eh_frame: 0xc0
-  __DATA_CONST.__auth_got: 0x3e0
-  __DATA_CONST.__got: 0xd8
-  __DATA_CONST.__auth_ptr: 0x80
-  __DATA_CONST.__const: 0xd88
-  __DATA_CONST.__objc_classlist: 0x10
+  __TEXT.__unwind_info: 0x238
+  __TEXT.__eh_frame: 0x1e8
+  __DATA_CONST.__auth_got: 0x538
+  __DATA_CONST.__got: 0x180
+  __DATA_CONST.__auth_ptr: 0x248
+  __DATA_CONST.__const: 0xe90
+  __DATA_CONST.__objc_classlist: 0x18
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA.__objc_const: 0x1a0
-  __DATA.__objc_selrefs: 0x68
-  __DATA.__objc_data: 0xa0
-  __DATA.__data: 0x290
-  __DATA.__common: 0x90
-  __DATA.__bss: 0x80
+  __DATA.__objc_const: 0x330
+  __DATA.__objc_selrefs: 0x90
+  __DATA.__objc_data: 0xf0
+  __DATA.__data: 0x5e0
+  __DATA.__bss: 0xa90
+  __DATA.__common: 0x8
   - /System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation
   - /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation
+  - /System/Library/PrivateFrameworks/ArgumentParserInternal.framework/Versions/A/ArgumentParserInternal
   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
   - /usr/lib/swift/libswiftCore.dylib

   - /usr/lib/swift/libswiftDispatch.dylib
   - /usr/lib/swift/libswiftIOKit.dylib
   - /usr/lib/swift/libswiftObjectiveC.dylib
+  - /usr/lib/swift/libswiftRegexBuilder.dylib
   - /usr/lib/swift/libswiftSystem.dylib
   - /usr/lib/swift/libswiftXPC.dylib
   - /usr/lib/swift/libswift_Builtin_float.dylib
-  - /usr/lib/swift/libswift_Concurrency.dylib
+  - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswift_errno.dylib
   - /usr/lib/swift/libswift_math.dylib
   - /usr/lib/swift/libswift_signal.dylib

   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsys_time.dylib
   - /usr/lib/swift/libswiftunistd.dylib
-  Functions: 72
-  Symbols:   189
-  CStrings:  167
+  Functions: 172
+  Symbols:   306
+  CStrings:  193
 
Symbols:
+ _$s10Foundation12CharacterSetV8newlinesACvgZ
+ _$s10Foundation3URLV15fileURLWithPath10relativeToACSSh_ACSghtcfC
+ _$s10Foundation3URLV15fileURLWithPathACSSh_tcfC
+ _$s10Foundation3URLV19_bridgeToObjectiveCSo5NSURLCyF
+ _$s10Foundation3URLV36_unconditionallyBridgeFromObjectiveCyACSo5NSURLCSgFZ
+ _$s10Foundation3URLV9appending9component13directoryHintACx_AC09DirectoryF0OtSyRzlF
+ _$s10Foundation3URLVs23CustomStringConvertibleAAMc
+ _$s12RegexBuilder0a9ComponentB0O15buildExpressionyxx17_StringProcessing0aC0RzlFZ
+ _$s12RegexBuilder0a9ComponentB0O17buildPartialBlock5first17_StringProcessing0A0Vy0A6OutputQzGx_tAF0aC0RzlFZ
+ _$s12RegexBuilder11makeFactory17_StringProcessing01_aD0VyF
+ _$s12RegexBuilder7CaptureVMn
+ _$s12RegexBuilder7CaptureVyACyxG17_StringProcessing0A0VyxGcfC
+ _$s12RegexBuilder7CaptureVyxG17_StringProcessing0A9ComponentAAMc
+ _$s15_ObjectiveCTypes01_A11CBridgeablePTl
+ _$s17_StringProcessing13_RegexFactoryV10accumulateyAA0C0VyxGq__q0_tAA0C9ComponentR_AaHR0_r1_lF
+ _$s17_StringProcessing13_RegexFactoryV7captureyAA0C0VyxGq_AA0C9ComponentR_r0_lF
+ _$s17_StringProcessing13_RegexFactoryVMa
+ _$s17_StringProcessing14RegexComponentP5regexAA0C0Vy0C6OutputQzGvgTj
+ _$s17_StringProcessing5RegexV06_regexA07versionACyxGSS_SitcfC
+ _$s17_StringProcessing5RegexV0C7BuilderEyACyxGqd__yXEc0C6OutputQyd__RszAA0C9ComponentRd__lufC
+ _$s17_StringProcessing5RegexV10wholeMatch2inAC0E0Vyx_GSgSs_tKF
+ _$s17_StringProcessing5RegexV11prefixMatch2inAC0E0Vyx_GSgSs_tKF
+ _$s17_StringProcessing5RegexV5MatchV13dynamicMemberqd__s7KeyPathCyxqd__G_tcluig
+ _$s17_StringProcessing5RegexV5MatchVMn
+ _$s17_StringProcessing5RegexVMn
+ _$s17_StringProcessing5RegexVyxGAA0C9ComponentAAMc
+ _$s22ArgumentParserInternal0A0V12wrappedValuexvg
+ _$s22ArgumentParserInternal0A0VA2A013ExpressibleByA0RzlE4help10completionACyxGAA0A4HelpVSg_AA14CompletionKindVSgtcfC
+ _$s22ArgumentParserInternal0A0VMa
+ _$s22ArgumentParserInternal0A0VMn
+ _$s22ArgumentParserInternal0A0VyxGSeAAMc
+ _$s22ArgumentParserInternal0A4HelpV13stringLiteralACSS_tcfC
+ _$s22ArgumentParserInternal0A4HelpVMa
+ _$s22ArgumentParserInternal0A4HelpVMn
+ _$s22ArgumentParserInternal14CompletionKindVMa
+ _$s22ArgumentParserInternal14CompletionKindVMn
+ _$s22ArgumentParserInternal15ParsableCommandMp
+ _$s22ArgumentParserInternal15ParsableCommandP12_commandNameSSvgZTq
+ _$s22ArgumentParserInternal15ParsableCommandP13configurationAA0E13ConfigurationVvgZTq
+ _$s22ArgumentParserInternal15ParsableCommandP3runyyKFTq
+ _$s22ArgumentParserInternal15ParsableCommandPAA0D9ArgumentsTb
+ _$s22ArgumentParserInternal15ParsableCommandPAAE12_commandNameSSvgZ
+ _$s22ArgumentParserInternal15ParsableCommandPAAE4mainyyFZ
+ _$s22ArgumentParserInternal17NameSpecificationV11customShort_14allowingJoinedACSJ_SbtFZ
+ _$s22ArgumentParserInternal17NameSpecificationVMa
+ _$s22ArgumentParserInternal17NameSpecificationVMn
+ _$s22ArgumentParserInternal17ParsableArgumentsMp
+ _$s22ArgumentParserInternal17ParsableArgumentsP11_errorLabelSSvgZTq
+ _$s22ArgumentParserInternal17ParsableArgumentsP8validateyyKFTq
+ _$s22ArgumentParserInternal17ParsableArgumentsPAAE11_errorLabelSSvgZ
+ _$s22ArgumentParserInternal17ParsableArgumentsPAAE8validateyyKF
+ _$s22ArgumentParserInternal17ParsableArgumentsPSeTb
+ _$s22ArgumentParserInternal17ParsableArgumentsPxycfCTq
+ _$s22ArgumentParserInternal20CommandConfigurationV11commandName8abstract5usage10discussion7version13shouldDisplay11subcommands17defaultSubcommand9helpNamesACSSSg_SSAMS2SSbSayAA08ParsableD0_pXpGAaN_pXpSgAA0G13SpecificationVSgtcfC
+ _$s22ArgumentParserInternal4FlagV12wrappedValuexvg
+ _$s22ArgumentParserInternal4FlagVAASbRszlE12wrappedValue4name4helpACySbGSb_AA17NameSpecificationVAA0A4HelpVSgtcfC
+ _$s22ArgumentParserInternal4FlagVMa
+ _$s22ArgumentParserInternal4FlagVMn
+ _$s22ArgumentParserInternal4FlagVyxGSeAAMc
+ _$s2os6LoggerVMn
+ _$s6System8FilePathV01_A11_FoundationEyACSg0D03URLVcfC
+ _$s8RawValueSYTl
+ _$sBbWV
+ _$sSD10FoundationE36_unconditionallyBridgeFromObjectiveCySDyxq_GSo12NSDictionaryCSgFZ
+ _$sSH13_rawHashValue4seedS2i_tFTq
+ _$sSH4hash4intoys6HasherVz_tFTq
+ _$sSH9hashValueSivgTq
+ _$sSHMp
+ _$sSHSQTb
+ _$sSQ2eeoiySbx_xtFZTq
+ _$sSQMp
+ _$sSS10FoundationE14contentsOfFile8encodingS2Sh_SSAAE8EncodingVtKcfC
+ _$sSS10FoundationE26_forceBridgeFromObjectiveC_6resultySo8NSStringC_SSSgztFZ
+ _$sSS10FoundationE34_conditionallyBridgeFromObjectiveC_6resultSbSo8NSStringC_SSSgztFZ
+ _$sSS14_fromSubstringySSSshFZ
+ _$sSS22ArgumentParserInternal013ExpressibleByA0AAWP
+ _$sSS4hash4intoys6HasherVz_tF
+ _$sSS9hashValueSivg
+ _$sSSSHsWP
+ _$sSSySsSnySS5IndexVGcig
+ _$sSY8rawValue03RawB0QzvgTq
+ _$sSY8rawValuexSg03RawB0Qz_tcfCTq
+ _$sSYMp
+ _$sSayxGSKsMc
+ _$sSbN
+ _$sSe4fromxs7Decoder_p_tKcfCTq
+ _$sSeMp
+ _$sSsN
+ _$ss20_SwiftNewtypeWrapperMp
+ _$ss20_SwiftNewtypeWrapperPSYTb
+ _$ss20_SwiftNewtypeWrapperPs35_HasCustomAnyHashableRepresentationTb
+ _$ss20_SwiftNewtypeWrapperPsSHRzSH8RawValueSYRpzrlE20_toCustomAnyHashables0hI0VSgyF
+ _$ss21_ObjectiveCBridgeableMp
+ _$ss21_ObjectiveCBridgeableP016_forceBridgeFromA1C_6resulty01_A5CTypeQz_xSgztFZTq
+ _$ss21_ObjectiveCBridgeableP024_conditionallyBridgeFromA1C_6resultSb01_A5CTypeQz_xSgztFZTq
+ _$ss21_ObjectiveCBridgeableP026_unconditionallyBridgeFromA1Cyx01_A5CTypeQzSgFZTq
+ _$ss21_ObjectiveCBridgeableP09_bridgeToA1C01_A5CTypeQzyFTq
+ _$ss22KeyedDecodingContainerV6decode_6forKeyqd__qd__m_xtKSeRd__lF
+ _$ss22KeyedDecodingContainerVMn
+ _$ss23CustomStringConvertibleMp
+ _$ss23CustomStringConvertibleP11descriptionSSvgTq
+ _$ss28CustomDebugStringConvertibleMp
+ _$ss28CustomDebugStringConvertibleP16debugDescriptionSSvgTq
+ _$ss35_HasCustomAnyHashableRepresentationMp
+ _$ss35_HasCustomAnyHashableRepresentationP03_tobcD0s0cD0VSgyFTq
+ _$ss6HasherV5_seedABSi_tcfC
+ _$ss6HasherV8_combineyySuF
+ _$ss6HasherV9_finalizeSiyF
+ _$ss7DecoderP9container7keyedBys22KeyedDecodingContainerVyqd__Gqd__m_tKs9CodingKeyRd__lFTj
+ _$ss9CodingKeyMp
+ _$ss9CodingKeyP11stringValueSSvgTq
+ _$ss9CodingKeyP11stringValuexSgSS_tcfCTq
+ _$ss9CodingKeyP8intValueSiSgvgTq
+ _$ss9CodingKeyP8intValuexSgSi_tcfCTq
+ _$ss9CodingKeyPs23CustomStringConvertibleTb
+ _$ss9CodingKeyPs28CustomDebugStringConvertibleTb
+ _$ss9CodingKeyPsE11descriptionSSvg
+ _$ss9CodingKeyPsE16debugDescriptionSSvg
+ _$sypN
+ _NSFileType
+ _NSFileTypeSymbolicLink
+ _swift_getEnumTagSinglePayloadGeneric
+ _swift_getForeignTypeMetadata
+ _swift_getKeyPath
+ _swift_initStructMetadata
+ _swift_storeEnumTagSinglePayloadGeneric
- _$sSa11descriptionSSvg
- _$sScA15unownedExecutorScevgTj
- _$sScM6sharedScMvgZ
- _$sScMMa
- _$sScMScAsWP
- _$ss17_assertionFailure__4file4line5flagss5NeverOs12StaticStringV_SSAHSus6UInt32VtF
- _swift_once
- _swift_task_isCurrentExecutor
- _swift_task_reportUnexpectedExecutor
CStrings:
+ "#\n# cat /var/run/racoon/*.conf\n#\n"
+ "/\"?([^\\s;\"]+)\"?/"
+ "/\\s+[AE]:\\s+\\S+\\s+/"
+ "/\\s+shared_secret\\s+use\\s+/"
+ "/sbin/route -n -v get -inet6 ::"
+ "/usr/sbin/skywalkctl flow-adv"
+ "/usr/sbin/skywalkctl flow-owner"
+ "/usr/sbin/skywalkctl flow-switch"
+ "_TtC16get_network_info12GNICollector"
+ "_collectNDFInformation"
+ "_collectSensitiveInformation"
+ "_collectSystemConfigurationFiles"
+ "_disablePacketCapture"
+ "attributesOfItemAtPath:error:"
+ "collectNDFInformation"
+ "collectSensitiveInformation"
+ "collectSystemConfigurationFiles"
+ "collects NDF information (sysctl, lsof)"
+ "collects sensitive information (ARP/NDP/mDNS cache, pcap)"
+ "collects system configuration files"
+ "copyItemAtURL:toURL:error:"
+ "dateFormatter"
+ "destinationOfSymbolicLinkAtPath:error:"
+ "disablePacketCapture"
+ "doesn't collect a packet capture"
+ "failed to create temporary file path at url '"
+ "fileHandleForUpdatingAtPath:"
+ "get-network-info"
+ "gniDirectory"
+ "gnisr"
+ "interfaceList"
+ "log collection tool for Networking information"
+ "logger"
+ "path to directory where all the information will be collected"
+ "pcapRunning"
+ "temporaryDirectory"
+ "tmp-gni-output.XXXXXX"
+ "tmp-redaction-stdin.txt"
- "/usr/bin/perl -l -n -e '\nif (/^(\\s+[AE]:\\s+\\S+\\s+)\"?(.*)\"?\\s*$/) {\n\tprintf \"%s[redacted]%s\\n\", $1, $3;\n} else {\n\tprintf \"%s\\n\", $_;\n}\n'"
- "/usr/bin/perl -l -n -e '\nif (/^(\\s+shared_secret\\s+use\\s+)\"?([^\\s;\"]+)\"?(.*)/) {\n\tprintf \"%s[redacted]%s\\n\", $1, $3;\n} else {\n\tprintf \"%s\\n\", $_;\n}\n'"
- "/usr/sbin/skywalkctl flow-adv -n"
- "/usr/sbin/skywalkctl flow-owner -n"
- "/usr/sbin/skywalkctl flow-switch -n"
- "ERROR: unrecognized argument(s) '"
- "Fatal error"
- "Usage: network-info [-c] [-n] [-s] [-P] <info-directory>\n -c\t\t    collects system configuration files\n -n\t\t    collects NDF information (lsof)\n -P\t\t    doesn't collect a packet capture\n -s\t\t    collects sensitive information (ARP/NDP/mDNS cache, pcap)\n <info-directory>   path to directory where all the information will be collected"
- "get_network_info/GNISubprocessRunner.swift"
- "get_network_info/main.swift"
- "tmp-gni-output.txt"
- "tmp-perl-stdin.txt"
```
