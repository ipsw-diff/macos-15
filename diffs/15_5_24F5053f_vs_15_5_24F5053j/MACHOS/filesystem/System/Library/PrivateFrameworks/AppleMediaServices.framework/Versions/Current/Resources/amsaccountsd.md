## amsaccountsd

> `/System/Library/PrivateFrameworks/AppleMediaServices.framework/Versions/Current/Resources/amsaccountsd`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__swift5_capture`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_types`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift5_mpenum`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`
- `__DATA.__objc_const`
- `__DATA.__objc_data`

```diff

-8.5.5.0.0
-  __TEXT.__text: 0x1aa450
+8.5.8.0.0
+  __TEXT.__text: 0x1afb24
   __TEXT.__auth_stubs: 0x2820
-  __TEXT.__objc_stubs: 0x7b40
+  __TEXT.__objc_stubs: 0x7b60
   __TEXT.__objc_methlist: 0x3a64
-  __TEXT.__const: 0x10288
+  __TEXT.__const: 0x103a8
   __TEXT.__objc_classname: 0xb62
-  __TEXT.__objc_methname: 0xaf00
-  __TEXT.__oslogstring: 0x7dbe
+  __TEXT.__objc_methname: 0xaf3c
+  __TEXT.__oslogstring: 0x7e4e
   __TEXT.__objc_methtype: 0x2ddb
-  __TEXT.__cstring: 0x72c8
+  __TEXT.__cstring: 0x73e8
   __TEXT.__gcc_except_tab: 0x574
   __TEXT.__dlopen_cstrs: 0x54
-  __TEXT.__swift5_typeref: 0x398d
-  __TEXT.__constg_swiftt: 0x34e0
-  __TEXT.__swift5_reflstr: 0x2bb0
-  __TEXT.__swift5_fieldmd: 0x4a48
+  __TEXT.__swift5_typeref: 0x39d5
+  __TEXT.__constg_swiftt: 0x34dc
+  __TEXT.__swift5_reflstr: 0x2bf0
+  __TEXT.__swift5_fieldmd: 0x4acc
   __TEXT.__swift5_capture: 0x5e4
   __TEXT.__swift5_builtin: 0x3c
   __TEXT.__swift5_assocty: 0x540
-  __TEXT.__swift5_proto: 0x1028
+  __TEXT.__swift5_proto: 0x1044
   __TEXT.__swift5_types: 0x4a8
   __TEXT.__swift_as_entry: 0x194
   __TEXT.__swift_as_ret: 0x230
   __TEXT.__swift5_protos: 0x7c
   __TEXT.__swift5_mpenum: 0x40
-  __TEXT.__unwind_info: 0x6328
-  __TEXT.__eh_frame: 0xad48
+  __TEXT.__unwind_info: 0x6380
+  __TEXT.__eh_frame: 0xae98
   __DATA_CONST.__auth_got: 0x1420
-  __DATA_CONST.__got: 0xca8
-  __DATA_CONST.__auth_ptr: 0xb30
-  __DATA_CONST.__const: 0x10e08
-  __DATA_CONST.__cfstring: 0x3360
+  __DATA_CONST.__got: 0xcb0
+  __DATA_CONST.__auth_ptr: 0xba8
+  __DATA_CONST.__const: 0x10f38
+  __DATA_CONST.__cfstring: 0x33a0
   __DATA_CONST.__objc_classlist: 0x278
   __DATA_CONST.__objc_catlist: 0x90
   __DATA_CONST.__objc_protolist: 0x1a0

   __DATA_CONST.__objc_superrefs: 0x138
   __DATA_CONST.__objc_intobj: 0x90
   __DATA.__objc_const: 0x7f78
-  __DATA.__objc_selrefs: 0x2b10
+  __DATA.__objc_selrefs: 0x2b18
   __DATA.__objc_ivar: 0x2c0
   __DATA.__objc_data: 0x18d8
-  __DATA.__data: 0x6d54
-  __DATA.__bss: 0x1f960
+  __DATA.__data: 0x6d14
+  __DATA.__bss: 0x1fce0
   __DATA.__common: 0x20c
   - /System/Library/Frameworks/Accounts.framework/Versions/A/Accounts
   - /System/Library/Frameworks/CloudKit.framework/Versions/A/CloudKit

   - /usr/lib/swift/libswiftsimd.dylib
   - /usr/lib/swift/libswiftsys_time.dylib
   - /usr/lib/swift/libswiftunistd.dylib
-  Functions: 9699
-  Symbols:   1306
-  CStrings:  3513
+  Functions: 9825
+  Symbols:   1307
+  CStrings:  3526
 
Symbols:
+ _ACErrorDomain
CStrings:
+ " from previous: \n"
+ "%{public}@: [%{public}@]Setting biometric state completed with error: %{public}@"
+ "%{public}@Will attempt to set biometrics on existing account"
+ ", isLowerPriority: "
+ "Final dataset group nextAction: "
+ "Next action for this account: "
+ "Skipping config DAS update"
+ "Skipping dataset DAS update"
+ "ams_accountWithAltDSID:DSID:username:accountTypeIdentifier:"
+ "com.apple.ams.accountdata"
+ "com.apple.ams.ts-datasync"
+ "lower_priority_datasets"
+ "x-apple-ts-config-version"
+ "x-apple-ts-id-domain"
+ "x-apple-ts-id-src"
- "Found dataset group scheduleNextSyncAt "
- "Skipping DAS update (isConfig: "
```
