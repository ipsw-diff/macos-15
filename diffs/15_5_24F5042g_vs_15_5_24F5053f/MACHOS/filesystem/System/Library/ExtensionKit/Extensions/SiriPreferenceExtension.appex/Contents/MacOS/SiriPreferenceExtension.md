## SiriPreferenceExtension

> `/System/Library/ExtensionKit/Extensions/SiriPreferenceExtension.appex/Contents/MacOS/SiriPreferenceExtension`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__const`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_types`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_entry`
- `__TEXT.__eh_frame`
- `__DATA_CONST.__cfstring`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA.__objc_const`
- `__DATA.__objc_selrefs`
- `__DATA.__objc_data`
- `__DATA.__data`

```diff

-3405.19.1.0.0
-  __TEXT.__text: 0x9d294
-  __TEXT.__auth_stubs: 0x26a0
+3405.23.1.4.1
+  __TEXT.__text: 0x9d568
+  __TEXT.__auth_stubs: 0x26b0
   __TEXT.__objc_stubs: 0x5c0
   __TEXT.__objc_methlist: 0x9fc
   __TEXT.__ustring: 0x10
-  __TEXT.__cstring: 0x5c04
+  __TEXT.__cstring: 0x5be4
   __TEXT.__objc_classname: 0x12c
   __TEXT.__objc_methname: 0x25d3
   __TEXT.__objc_methtype: 0x520
   __TEXT.__const: 0x53f4
   __TEXT.__oslogstring: 0xd8
-  __TEXT.__swift5_typeref: 0x8790
-  __TEXT.__swift5_capture: 0xb2c
+  __TEXT.__swift5_typeref: 0x8770
+  __TEXT.__swift5_capture: 0xb54
   __TEXT.__constg_swiftt: 0x2e30
   __TEXT.__swift5_reflstr: 0x2065
   __TEXT.__swift5_fieldmd: 0x1258
   __TEXT.__swift5_proto: 0x1d4
   __TEXT.__swift5_types: 0x150
-  __TEXT.__swift_as_entry: 0x88
-  __TEXT.__swift_as_ret: 0x80
+  __TEXT.__swift_as_entry: 0x8c
+  __TEXT.__swift_as_ret: 0x84
   __TEXT.__swift5_assocty: 0x740
   __TEXT.__swift5_builtin: 0x78
   __TEXT.__swift5_entry: 0x8
   __TEXT.__swift5_protos: 0x10
-  __TEXT.__unwind_info: 0x2938
+  __TEXT.__unwind_info: 0x2948
   __TEXT.__eh_frame: 0x15e8
-  __DATA_CONST.__auth_got: 0x1358
-  __DATA_CONST.__got: 0xa90
-  __DATA_CONST.__auth_ptr: 0xfa0
-  __DATA_CONST.__const: 0x3bd0
+  __DATA_CONST.__auth_got: 0x1360
+  __DATA_CONST.__got: 0xa98
+  __DATA_CONST.__auth_ptr: 0xfb0
+  __DATA_CONST.__const: 0x3c20
   __DATA_CONST.__cfstring: 0x500
   __DATA_CONST.__objc_classlist: 0x98
   __DATA_CONST.__objc_protolist: 0xd0

   - /usr/lib/swift/libswiftsimd.dylib
   - /usr/lib/swift/libswiftsys_time.dylib
   - /usr/lib/swift/libswiftunistd.dylib
-  Functions: 3651
+  Functions: 3656
   Symbols:   275
   CStrings:  1041
 
CStrings:
+ "Error setting GMS opt-in status: "
- "#GMS Availability: user already signed up, skip updating local value"
```
